#!/usr/bin/env python3
"""
Standalone Dating Show Simulation
Runs using only @dating_show/ and @dating_show_env/ components
Does not require the separate @reverie/ directory
"""
...
# Add paths for local reverie imports
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))
sys.path.insert(0, str(project_root / "dating_show" / "reverie_core"))

# Import from local reverie_core
try:
    from reverie_core.global_methods import fs_storage
    from reverie_core.reverie import ReverieServer
    REVERIE_AVAILABLE = True
    print("✅ Local reverie core successfully imported")
    print(f"📁 Storage path: {fs_storage}")
except Exception as e:
    print(f"❌ Local reverie import failed: {e}")
    import traceback
    traceback.print_exc()
    REVERIE_AVAILABLE = False


class StandaloneDatingShowSimulation:
    """
    Standalone simulation runner using only dating_show and dating_show_env
    """
    ...
    def __init__(self, fork_sim_code="base_the_ville_n25", sim_code="dating_show_25_agents"):
        ...
        print(f"🎭 Standalone Dating Show Simulation")
    ...
    def create_temp_storage_files(self, step=1):
        ...
        try:
            # Create temp_storage directory in dating_show_env
    ...
    def initialize_simulation(self):
        ...
        if not REVERIE_AVAILABLE:
            print("❌ Local reverie core not available - cannot initialize simulation")
            return False
        ...
        try:
            ...
            # Create reverie server using local reverie_core
...
def main():
    ...
    print("Using only @dating_show/ and @dating_show_env/")
    print("=" * 50)
    
    # Check availability
    print(f"Local reverie core available: {'✅' if REVERIE_AVAILABLE else '❌'}")
    
    if not REVERIE_AVAILABLE:
        print("\n❌ Local reverie core not available!")
        print("Please ensure reverie_core directory exists with required files.")
        return 1
    
    # Create runner
    runner = StandaloneDatingShowSimulation()
