...
def api_agent_detail(request, agent_id):
    """GET /api/agents/{agent_id}/ - Get detailed agent information"""
    agent = get_object_or_404(Agent, agent_id=agent_id)
...
def api_agent_state_update(request, agent_id):
    """POST/PUT /api/agents/{agent_id}/state/ - Update agent state from PIANO engine"""
    try:
...
def api_social_network(request):
    """GET /api/social/network/ - Get social network data for visualization"""
    agents = Agent.objects.all()
    relationships = SocialRelationship.objects.all().select_related('agent_a', 'agent_b')
